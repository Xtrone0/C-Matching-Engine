...
TEST_CASE(passive_orders_both_sides) {
    for (Side side : {Side::Buy, Side::Sell}) {
        ...
        if (side == Side::Buy) check_best(book, 100, std::nullopt);
        else check_best(book, std::nullopt, 100);
    }
}
...
TEST_CASE(best_price_updates_after_cancels_both_sides) {
    for (Side side : {Side::Buy, Side::Sell}) {
        ...
        auto check = [&](std::optional<Price> expected) {
            if (side == Side::Buy) check_best(book, expected, std::nullopt);
            else check_best(book, std::nullopt, expected);
        };
        check(best);
        CHECK(book.cancel(2)); check(100);
        CHECK(book.cancel(3)); check(worst);
        CHECK(book.cancel(1)); check(std::nullopt);
    }
}
...
TEST_CASE(fifo_uses_arrival_not_id_both_sides) {
    for (Side side : sides) {
        ...
        check_trades(book.submit(order(50, side, 100, 6)), {
            execution(side, 50, 90, 100, 2), execution(side, 50, 10, 100, 3),
...
TEST_CASE(differential_limit_cancel_seeded_streams) {
    for (std::uint64_t seed : {42u, 598u}) differential(seed, false);
    for (std::uint64_t seed : {42u, 598u, 2026u}) differential(seed, true);
}
...
TEST_CASE(lesson3_partial_fill_and_reuse_preserve_arrival_order) {
    for (Side side : sides) {
        ...
        check_trades(book.submit(order(30, opposite(side), 100, 4)), {
            execution(opposite(side), 30, 10, 100, 3),
...
TEST_CASE(mixed_differential_seeded_streams) {
    for (std::uint64_t seed : {6u, 491u, 2026u}) mixed_differential(seed);
}
