...
namespace
{
    ...
    struct Register
    {
        Register(std::string_view name, void (*run)())
        {
...
#define CHECK(expression)                                                          \
    ...
    // Core book state and larger stress scenarios.
    ...
    TEST_CASE(passive_orders_both_sides)
    {
        for (Side side : {Side::Buy, Side::Sell})
        {
            CheckedOrderBook book;
            CHECK(book.submit({1, side, 100, 5}).empty());
            if (side == Side::Buy)
    ...
    TEST_CASE(non_crossing_orders_both_sides)
    {
        for (Side incoming : {Side::Buy, Side::Sell})
        {
            CheckedOrderBook book;
            if (incoming == Side::Buy)
            {
                CHECK(book.submit(sell(1, 101, 5)).empty());
                CHECK(book.submit(buy(2, 100, 5)).empty());
            }
            else
            {
    ...
    TEST_CASE(best_price_updates_after_cancels_both_sides)
    {
        for (Side side : {Side::Buy, Side::Sell})
        {
            CheckedOrderBook book;
            const Price worst = side == Side::Buy ? 99 : 101;
            const Price best = side == Side::Buy ? 101 : 99;
            book.submit({1, side, worst, 1});
            book.submit({2, side, best, 1});
            book.submit({3, side, 100, 1});
            auto check = [&](std::optional<Price> expected)
            {
                if (side == Side::Buy)
                    check_best(book, expected, std::nullopt);
                else
                    check_best(book, std::nullopt, expected);
            };
            check(best);
            CHECK(book.cancel(2));
            check(100);
    ...
    TEST_CASE(test_many_orders_same_price)
    {
    ...
    TEST_CASE(test_many_price_levels)
    {
        constexpr int N = 100;
    ...
    TEST_CASE(test_randomized_invariants)
    {
    ...
    // Boundary, reference-model, and invariant regressions.
    ...
    Order order(OrderId id, Side side, Price p, Quantity q)
    {
        return {id, side, p, q};
    }
    ...
    TEST_CASE(resting_price_used_both_sides)
    {
        for (Side side : sides)
        {
            for (OrderId maker : {OrderId{1}, OrderId{100}})
            {
                CheckedOrderBook book;
                const OrderId taker = maker == 1 ? 2 : 1;
                const Price p = price(side, -2);
                book.submit(order(maker, opposite(side), p, 5));
                check_trades(book.submit(order(taker, side, 100, 5)),
                             {execution(side, taker, maker, p, 5)});
                check_empty(book);
            }
        }
    }
    TEST_CASE(fifo_uses_arrival_not_id_both_sides)
    {
        ...
        {
            CheckedOrderBook book;
            book.submit(order(90, opposite(side), 100, 2));
            book.submit(order(10, opposite(side), 100, 3));
            book.submit(order(70, opposite(side), 100, 4));
            check_trades(book.submit(order(50, side, 100, 6)), {execution(side, 50, 90, 100, 2), execution(side, 50, 10, 100, 3),
                                                                execution(side, 50, 70, 100, 1)});
            check_trades(book.submit(order(60, side, 100, 3)), {execution(side, 60, 70, 100, 3)});
            check_empty(book);
        }
    }
    TEST_CASE(sweep_prices_with_shuffled_ids_both_sides)
    {
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(50, side, price(side, 3), 9)), {execution(side, 50, 70, price(side, 0), 2),
    ...
    TEST_CASE(partial_maker_keeps_fifo_priority_both_sides)
    {
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(5, side, 100, 8)), {execution(side, 5, 1, 100, 3), execution(side, 5, 2, 100, 4),
    ...
    TEST_CASE(residual_rests_at_own_limit_both_sides)
    {
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(2, side, price(side, 2), 5)), {execution(side, 2, 1, price(side, 0), 2)});
            CHECK((side == Side::Buy ? book.best_bid() : book.best_ask()) == price(side, 2));
            check_trades(book.submit(order(3, opposite(side), price(side, 1), 3)), {execution(opposite(side), 3, 2, price(side, 2), 3)});
            check_empty(book);
        }
    }
    TEST_CASE(later_order_joins_behind_partial_maker_both_sides)
    {
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(3, opposite(side), 100, 5)), {execution(opposite(side), 3, 1, 100, 3), execution(opposite(side), 3, 2, 100, 2)});
            book.submit(order(4, side, 100, 1));
            check_trades(book.submit(order(5, opposite(side), 100, 3)), {execution(opposite(side), 5, 2, 100, 2), execution(opposite(side), 5, 4, 100, 1)});
            check_empty(book);
        }
    }
    TEST_CASE(sweep_stops_at_limit_with_both_sides_remaining)
    {
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(3, side, price(side, 1), 5)), {execution(side, 3, 1, price(side, 0), 2)});
            ...
            CHECK(book.cancel(3));
            check_trades(book.submit(order(4, side, price(side, 2), 7)), {execution(side, 4, 2, price(side, 2), 7)});
            check_empty(book);
        }
    }
    TEST_CASE(exact_level_exhaustion_leaves_worse_level_untouched)
    {
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(4, side, price(side, 2), 5)), {execution(side, 4, 1, price(side, 0), 2), execution(side, 4, 2, price(side, 0), 3)});
            CHECK((side == Side::Buy ? book.best_ask() : book.best_bid()) == price(side, 1));
            check_trades(book.submit(order(5, side, price(side, 1), 7)), {execution(side, 5, 3, price(side, 1), 7)});
            check_empty(book);
        }
    }
    TEST_CASE(roadmap_sweep_example_and_mirror)
    {
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(5, side, price(side, 2), 17)), {execution(side, 5, 1, price(side, 1), 4), execution(side, 5, 2, price(side, 1), 6),
                                                                           execution(side, 5, 3, price(side, 2), 7)});
            CHECK(book.submit(order(6, side, price(side, 1), 1)).empty());
            CHECK(book.cancel(6));
            check_trades(book.submit(order(7, side, price(side, 3), 23)), {execution(side, 7, 3, price(side, 2), 3), execution(side, 7, 4, price(side, 3), 20)});
            check_empty(book);
        }
    }
    void invalid_quantity(Quantity quantity, bool crossing)
    {
        for (Side side : sides)
        {
            ...
            expect_rejection([&]
                             {
                                 book.submit(order(3, side, price(side, crossing ? 1 : -1), quantity));
                             });
            CHECK(!book.cancel(3));
            check_trades(book.submit(order(4, side, 100, 7)), {execution(side, 4, 1, 100, 3), execution(side, 4, 2, 100, 4)});
            check_empty(book);
        }
    }
    TEST_CASE(invalid_limit_quantities_rejected_without_mutation)
    {
        for (Quantity quantity : {Quantity{0}, max_quantity + 1, std::numeric_limits<Quantity>::max()})
            for (bool crossing : {false, true})
                invalid_quantity(quantity, crossing);
    }
    TEST_CASE(invalid_sides_rejected_without_mutation)
    {
        for (int raw : {-1, 2, 127})
        {
            ...
            expect_rejection([&]
                             {
                                 book.submit(order(3, static_cast<Side>(raw), 100, 5));
                             });
            CHECK(!book.cancel(3));
    ...
    void duplicate_order(bool other_side, bool other_price, bool partially_filled)
    {
        for (Side side : sides)
        {
            ...
            if (partially_filled)
                check_trades(book.submit(order(2, opposite(side), 100, 2)), {execution(opposite(side), 2, 1, 100, 2)});
            const Side duplicate_side = other_side ? opposite(side) : side;
            expect_rejection([&]
                             {
                                 book.submit(order(1, duplicate_side, other_price ? 101 : 100, 9));
                             });
            check_trades(book.submit(order(3, opposite(side), 100, partially_filled ? 3 : 5)), {execution(opposite(side), 3, 1, 100, partially_filled ? 3 : 5)});
            CHECK(!book.cancel(1));
            check_empty(book);
        }
    }
    TEST_CASE(duplicate_active_limit_ids_rejected)
    {
        duplicate_order(false, false, false); // Same level.
        duplicate_order(false, true, false);  // Different level.
        duplicate_order(true, false, false);  // Opposite side, before matching.
        duplicate_order(true, false, true);   // Partially filled maker.
    }
    TEST_CASE(rejected_submission_does_not_reserve_id)
    {
        for (Side side : sides)
        {
            ...
            expect_rejection([&]
                             {
                                 book.submit(order(1, side, 100, 0));
                             });
            CHECK(book.submit(order(1, side, 100, 3)).empty());
            check_trades(book.submit(order(2, opposite(side), 100, 3)), {execution(opposite(side), 2, 1, 100, 3)});
            check_empty(book);
        }
    }
    TEST_CASE(invalid_orders_rejected_from_empty_book)
    {
        for (Side side : sides)
        {
            for (Quantity q : {Quantity{0}, max_quantity + 1, std::numeric_limits<Quantity>::max()})
            {
                ...
                expect_rejection([&]
                                 {
                                     book.submit(order(1, side, 100, q));
                                 });
                CHECK(!book.cancel(1));
                check_empty(book);
            }
        ...
        expect_rejection([&]
                         {
                             book.submit(order(1, static_cast<Side>(2), 100, 1));
                         });
        check_empty(book);
    }
    ...
    TEST_CASE(cancel_nonbest_level_keeps_best_both_sides)
    {
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(3, side, price(side, 1), 2)), {execution(side, 3, 1, price(side, 0), 2)});
            check_empty(book);
        }
    }
    ...
    TEST_CASE(maximum_allowed_quantity_partial_fill)
    {
        ...
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(2, side, 100, maximum - 1)), {execution(side, 2, 1, 100, maximum - 1)});
    ...
    TEST_CASE(maximum_allowed_quantity_multi_fill)
    {
        ...
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(3, side, 100, maximum)), {execution(side, 3, 1, 100, maximum - 1), execution(side, 3, 2, 100, 1)});
            check_empty(book);
        }
    }
    TEST_CASE(maximum_order_id_is_not_arrival_time)
    {
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(1, side, price(side, 1), 1)), {execution(side, 1, id, price(side, 0), 1)});
            check_empty(book);
        }
    }
    void incoming_large_id_uses_resting_prices(Side side, OrderId incoming_id)
    {
        ...
        check_trades(book.submit(order(incoming_id, side, 100, 5)), {execution(side, incoming_id, 1, first_price, 2),
    ...
    TEST_CASE(incoming_large_ids_use_resting_prices_both_sides)
    {
        for (Side side : sides)
        {
            for (OrderId id : {
                ...
                incoming_large_id_uses_resting_prices(side, id);
        }
    }
    ...
    TEST_CASE(differential_limit_cancel_seeded_streams)
    {
        ...
        for (std::uint64_t seed : {42u, 598u, 2026u})
    ...
    TEST_CASE(market_empty_book_expires_and_releases_id)
    {
        for (Side side : sides)
        {
            CheckedOrderBook book;
    ...
    TEST_CASE(market_does_not_match_same_side_both_sides)
    {
        for (Side side : sides)
        {
            ...
            CHECK(!book.cancel(2));
            check_trades(book.submit(order(3, opposite(side), 100, 3)), {execution(opposite(side), 3, 1, 100, 3)});
            check_empty(book);
        }
    }
    TEST_CASE(market_partial_maker_keeps_remaining_fifo)
    {
        for (Side side : sides)
        {
            ...
            CHECK(!book.cancel(1));
            check_trades(market(book, 2, side, 6), {execution(side, 2, 90, 100, 3), execution(side, 2, 10, 100, 3)});
            check_empty(book);
        }
    }
    TEST_CASE(market_exact_liquidity_removes_all_levels)
    {
        for (Side side : sides)
        {
            ...
            check_trades(market(book, 1, side, 5), {execution(side, 1, 90, price(side, 0), 2), execution(side, 1, 70, price(side, 2), 3)});
            CHECK(!book.cancel(1));
    ...
    TEST_CASE(market_sweeps_fifo_and_prices_then_expires_residual)
    {
        for (Side side : sides)
        {
            ...
            check_trades(market(book, 1, side, 12), {execution(side, 1, 70, price(side, 0), 2), execution(side, 1, 10, price(side, 0), 3),
    ...
    TEST_CASE(market_invalid_side_rejected_without_mutation)
    {
        ...
        expect_rejection([&]
                         {
                             market(book, 2, static_cast<Side>(2), 3);
                         });
    ...
    TEST_CASE(cancel_preserves_survivor_priorities_and_fifo)
    {
        for (Side side : sides)
        {
            ...
            const std::vector<Scenario> scenarios = {
                {90, 14, {{10, side, 100, 6, 2}, {70, side, 100, 8, 3}}, {execution(taker, 1000, 10, 100, 6), execution(taker, 1000, 70, 100, 8)}},
                {10, 12, {{90, side, 100, 4, 1}, {70, side, 100, 8, 3}}, {execution(taker, 1000, 90, 100, 4), execution(taker, 1000, 70, 100, 8)}},
                {70, 10, {{90, side, 100, 4, 1}, {10, side, 100, 6, 2}}, {execution(taker, 1000, 90, 100, 4), execution(taker, 1000, 10, 100, 6)}}};
            ...
            for (const auto &scenario : scenarios)
            {
                ...
                CHECK(!OrderBookTestAccess::ids(book).contains(scenario.canceled));
                for (const auto &[id, location] : OrderBookTestAccess::ids(book))
                {
                    CHECK(std::addressof(*location.level) == std::addressof(*saved.at(id).level));
                    CHECK(std::addressof(*location.order) == std::addressof(*saved.at(id).order));
                }
    ...
    TEST_CASE(lesson3_valid_price_and_quantity_boundaries)
    {
        for (Side side : sides)
        {
            for (Price p : {Price{1}, Price{100}, max_price})
            {
                for (Quantity q : {Quantity{1}, Quantity{5}, max_quantity})
                {
                    CheckedOrderBook book;
                    ...
                    (side == Side::Buy ? expected.bids : expected.asks) = {
                        {p, q, {{1, side, p, q, 1}}}};
                    check_snapshot(book.snapshot(), expected);
                    check_trades(book.submit(order(2, opposite(side), p, q)), {execution(opposite(side), 2, 1, p, q)});
                    check_snapshot(book.snapshot(), Snapshot{});
                }
            }
        }
    ...
    TEST_CASE(lesson3_partial_fill_and_reuse_preserve_arrival_order)
    {
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(20, opposite(side), 100, 2)), {execution(opposite(side), 20, 90, 100, 2)});
            ...
            check_trades(book.submit(order(30, opposite(side), 100, 4)), {execution(opposite(side), 30, 10, 100, 3),
                                                                          execution(opposite(side), 30, 90, 100, 1)});
            check_empty(book);
        }
    }
    template <class Book>
    Depth depth(const Book &book)
    {
        if constexpr (requires { book.snapshot().bids; book.snapshot().asks; })
        {
            ...
            for (const auto &level : snapshot.bids)
                result.first.emplace_back(level.price, level.quantity);
            for (const auto &level : snapshot.asks)
                result.second.emplace_back(level.price, level.quantity);
            return result;
        }
        else
        {
    ...
    TEST_CASE(snapshot_aggregates_and_sorts_both_sides)
    {
        ...
        Snapshot full;
        full.bids = {
            {99, 7, {{2, Side::Buy, 99, 3, 2}, {3, Side::Buy, 99, 4, 3}}},
            {98, 2, {{1, Side::Buy, 98, 2, 1}}}};
        full.asks = {
            {101, 13, {{5, Side::Sell, 101, 6, 5}, {6, Side::Sell, 101, 7, 6}}},
            {102, 5, {{4, Side::Sell, 102, 5, 4}}}};
        check_snapshot(book.snapshot(), full);
        check_snapshot(book.snapshot(), full);
        CHECK(book.best_bid() == 99);
        CHECK(book.best_ask() == 101);
    ...
    TEST_CASE(snapshot_updates_after_partial_fill_cancel_and_removal)
    {
        for (Side side : sides)
        {
            ...
            book.submit(order(1, side, 100, 5));
            book.submit(order(2, side, 100, 4));
            book.submit(order(3, side, worse, 2));
            auto expectDepth = [&](const Levels &levels)
            {
                Depth expected;
                (side == Side::Buy ? expected.first : expected.second) = levels;
                check_depth(depth(book), expected);
            };
            expectDepth({{100, 9}, {worse, 2}});
            book.submit(order(4, opposite(side), 100, 3));
            expectDepth({{100, 6}, {worse, 2}});
            CHECK(book.cancel(2));
            expectDepth({{100, 2}, {worse, 2}});
            book.submit(order(5, opposite(side), 100, 2));
            expectDepth({{worse, 2}});
            CHECK(book.cancel(3));
            check_depth(depth(book), Depth{});
        }
    ...
    TEST_CASE(lesson6_saved_mixed_command_fixture)
    {
        ...
        constexpr std::array<Command, 20> commands{{{Action::Limit, 1, Side::Buy, 99, 4},
        ...
        for (std::size_t i = 0; i < commands.size(); ++i)
        {
            ...
            if (i == 6)
                expected = {{6, 4, 101, 3}, {6, 5, 102, 2}};
            if (i == 8)
                expected = {{2, 7, 99, 5}};
            if (i == 11)
                expected = {{3, 8, 98, 8}, {2, 8, 98, 2}};
            if (i == 17)
                expected = {{10, 5, 102, 3}, {10, 8, 103, 1}};
    ...
    TEST_CASE(lesson6_workbook_cancel_then_partial_fill)
    {
        for (Side side : sides)
        {
            ...
            CHECK(book.cancel(2));
            check_trades(book.submit(order(4, opposite(side), 100, 5)), {execution(opposite(side), 4, 1, 100, 4), execution(opposite(side), 4, 3, 100, 1)});
            Snapshot expected;
            (side == Side::Buy ? expected.bids : expected.asks) = {
    ...
    TEST_CASE(lesson6_fills_and_expiration_do_not_consume_arrival_priority)
    {
        for (Side side : sides)
        {
            ...
            expect_rejection([&]
                             {
                                 book.submit(order(4, side, 100, 0));
                             });
    ...
    TEST_CASE(lesson6_repeated_level_recreation_and_id_reuse)
    {
        for (Side side : sides)
        {
            ...
            for (int cycle = 0; cycle < 100; ++cycle)
            {
                ...
                {
                    check_trades(book.submit_market(3, opposite(side), 10), {execution(opposite(side), 3, 1, 100, 3),
    ...
    void mixed_differential(std::uint64_t seed)
    {
        ...
        for (std::size_t step = 0; step < 1000; ++step)
        {
            ...
            case 0:
                ...
                break;
            case 1:
                command.id = std::numeric_limits<OrderId>::max();
                break;
            case 2:
                command.side = static_cast<Side>(2);
                break;
            case 3:
                command.quantity = 0;
                break;
            case 4:
                command.quantity = max_quantity + 1;
                break;
            case 5:
                command.price = 0;
                break;
            case 6:
                command.price = max_price + 1;
                break;
            default:
                break;
            }
    ...
    TEST_CASE(mixed_differential_seeded_streams)
    {
        for (std::uint64_t seed : {6u, 491u, 2026u})
            mixed_differential(seed);
    }
        ...
        for (std::size_t length = 1; length <= 4; ++length)
        {
            ...
            for (std::size_t trace = 0; trace < count; ++trace)
            {
                ...
                for (std::size_t step = 0; step < length; ++step)
                {
                    ...
                    {
                        throw std::runtime_error("length=" + std::to_string(length) + " trace=" + std::to_string(trace) + " " + error.what());
                    }
                }
            }
        }
    ...
    TEST_CASE(invariants_checked_add_rejects_overflow)
    {
        ...
        for (const auto &values : std::vector<std::pair<Quantity, Quantity>>{
                 {maximum, 1}, {maximum - 1, 2}, {maximum, maximum}, {maximum - max_quantity + 1, max_quantity}})
        {
            expect_logic_error([&]
                               {
                                   OrderBookTestAccess::add(values.first, values.second);
                               },
    ...
    TEST_CASE(locations_partial_fill_keeps_maker_node_both_sides)
    {
        for (Side side : sides)
        {
            ...
            check_trades(book.submit(order(4, opposite(side), 100, 7)),
                         {execution(opposite(side), 4, 2, 100, 7)});
            CHECK(!book.cancel(2));
            check_empty(book);
        }
    }
        ...
        for (Side taker : sides)
        {
            for (bool market : {false, true})
            {
                ...
                else
                {
                    ...
                    Snapshot expected;
                    auto &levels = taker == Side::Buy ? expected.bids : expected.asks;
                    levels.push_back(SnapshotLevel{
                        price(taker, 2), 2, {Order{3, taker, price(taker, 2), 2, 3}}});
                    check_snapshot(book.snapshot(), expected);
                    CHECK(book.cancel(3));
                }
                check_empty(book);
    ...
    {
        reject_bad_location([](auto &active, Side side)
                            {
                                active.at(1).side = opposite(side);
                            });
    }
    TEST_CASE(locations_checker_rejects_invalid_side)
    {
        reject_bad_location([](auto &active, Side)
                            {
                                active.at(1).side = static_cast<Side>(99);
                            });
    }
    TEST_CASE(locations_checker_rejects_other_level_same_side)
    {
        reject_bad_location([](auto &active, Side)
                            {
                                active.at(1).level = active.at(3).level;
                            });
    }
    TEST_CASE(locations_checker_rejects_level_on_opposite_side)
    {
        reject_bad_location([](auto &active, Side)
                            {
                                active.at(1).level = active.at(4).level;
                            });
    }
    TEST_CASE(locations_checker_rejects_other_order_same_level)
    {
        reject_bad_location([](auto &active, Side)
                            {
                                active.at(1).order = active.at(2).order;
                            });
    }
    TEST_CASE(locations_checker_rejects_order_at_other_level)
    {
        reject_bad_location([](auto &active, Side)
                            {
                                active.at(1).order = active.at(3).order;
                            });
    }
    TEST_CASE(locations_checker_rejects_order_on_opposite_side)
    {
        reject_bad_location([](auto &active, Side)
                            {
                                active.at(1).order = active.at(4).order;
                            });
    }
    TEST_CASE(locations_checker_rejects_swapped_complete_locations)
    {
        reject_bad_location([](auto &active, Side)
                            {
                                std::swap(active.at(1), active.at(2));
                            });
    }
// Amendment scenarios use literal outcomes, independently of ReferenceBook.
void reject_amend_unchanged(CheckedOrderBook& book, OrderId id, Price p, Quantity q) {
    const auto before = book.snapshot();
    const auto priority = OrderBookTestAccess::counter(book);
    const auto locations = OrderBookTestAccess::ids(book);
    expect_rejection([&] { book.amend(id, p, q); });
    check_snapshot(book.snapshot(), before);
    CHECK(OrderBookTestAccess::counter(book) == priority);
    CHECK(OrderBookTestAccess::ids(book).size() == locations.size());
    for (const auto& [savedId, saved] : locations) {
        const auto& current = OrderBookTestAccess::ids(book).at(savedId);
        CHECK(current.side == saved.side);
        CHECK(current.level == saved.level);
        CHECK(current.order == saved.order);
    }
}

TEST_CASE(amend_unknown_and_inactive_ids_preserve_state) {
    for (Side side : sides) {
        CheckedOrderBook book;
        for (OrderId id : {OrderId{0}, OrderId{999}, std::numeric_limits<OrderId>::max()})
            reject_amend_unchanged(book, id, 100, 1);
        book.submit(order(1, side, 100, 3));
        book.submit(order(2, side, 100, 5));
        CHECK(book.cancel(1));
        reject_amend_unchanged(book, 1, 100, 1);
        check_trades(book.submit(order(3, opposite(side), 100, 5)),
                     {execution(opposite(side), 3, 2, 100, 5)});
        book.submit(order(4, side, 100, 2));
        for (OrderId id : {OrderId{0}, OrderId{2}, OrderId{3}, OrderId{999}})
            reject_amend_unchanged(book, id, 100, 1);
        CHECK(book.cancel(4));
    }
}

TEST_CASE(amend_invalid_prices_preserve_state) {
    for (Side side : sides) {
        CheckedOrderBook book;
        book.submit(order(1, side, 100, 6));
        book.submit(order(2, side, 100, 5));
        for (Price p : {std::numeric_limits<Price>::min(), Price{-1}, Price{0},
                        max_price + 1, std::numeric_limits<Price>::max()})
            for (Quantity q : {Quantity{3}, Quantity{6}, Quantity{8}})
                reject_amend_unchanged(book, 1, p, q);
        check_trades(book.submit(order(3, opposite(side), 100, 7)), {
            execution(opposite(side), 3, 1, 100, 6), execution(opposite(side), 3, 2, 100, 1)});
    }
}

TEST_CASE(amend_zero_quantity_preserves_state) {
    for (Side side : sides) {
        CheckedOrderBook book;
        book.submit(order(1, side, 100, 6));
        book.submit(order(2, side, 100, 5));
        book.submit(order(3, opposite(side), price(side, 2), 4));
        for (Price p : {Price{100}, price(side, -1), price(side, 2)})
            reject_amend_unchanged(book, 1, p, 0);
        CHECK(book.cancel(1));
        check_location(book, 2, side, 100);
    }
}

void excessive_amend_quantity(Price offset) {
    for (Side side : sides) {
        for (Quantity q : {max_quantity + 1, std::numeric_limits<Quantity>::max()}) {
            CheckedOrderBook book;
            book.submit(order(1, side, 100, 6));
            book.submit(order(2, side, 100, 5));
            book.submit(order(3, opposite(side), price(side, 2), 4));
            reject_amend_unchanged(book, 1, price(side, offset), q);
            check_trades(book.submit(order(4, opposite(side), 100, 7)), {
                execution(opposite(side), 4, 1, 100, 6), execution(opposite(side), 4, 2, 100, 1)});
        }
    }
}
TEST_CASE(amend_excessive_quantity_same_price_preserves_state) {
    excessive_amend_quantity(0);
}
TEST_CASE(amend_excessive_quantity_passive_reprice_preserves_state) {
    excessive_amend_quantity(-1);
}
TEST_CASE(amend_excessive_quantity_crossing_reprice_preserves_state) {
    excessive_amend_quantity(2);
}

TEST_CASE(amend_noop_and_reduction_preserve_nodes_priority_and_fifo) {
    for (Side side : sides) {
        for (OrderId target : {OrderId{90}, OrderId{10}, OrderId{70}}) {
            for (Quantity q : {Quantity{6}, Quantity{3}}) {
                CheckedOrderBook book;
                for (OrderId id : {OrderId{90}, OrderId{10}, OrderId{70}})
                    book.submit(order(id, side, 100, 6));
                const auto locations = OrderBookTestAccess::ids(book);
                auto expected = book.snapshot();
                auto& level = (side == Side::Buy ? expected.bids : expected.asks).front();
                level.quantity -= 6 - q;
                for (auto& item : level.orders)
                    if (item.id == target) item.quantity = q;
                CHECK(book.amend(target, 100, q).empty());
                check_snapshot(book.snapshot(), expected);
                CHECK(OrderBookTestAccess::counter(book) == 3);
                for (const auto& [id, saved] : locations) {
                    CHECK(OrderBookTestAccess::ids(book).at(id).level == saved.level);
                    CHECK(OrderBookTestAccess::ids(book).at(id).order == saved.order);
                }
                std::vector<Trade> trades;
                for (OrderId id : {OrderId{90}, OrderId{10}, OrderId{70}})
                    trades.push_back(execution(opposite(side), 1000, id, 100, id == target ? q : 6));
                check_trades(book.submit(order(1000, opposite(side), 100, 12 + q)), trades);
                check_snapshot(book.snapshot(), Snapshot{});
            }
        }
    }
...
TEST_CASE(amend_after_partial_fill_uses_remaining_and_applies_fifo_rules) {
    for (Side side : sides) {
        for (Quantity q : {Quantity{3}, Quantity{6}, Quantity{8}}) {
            CheckedOrderBook book;
            book.submit(order(1, side, 100, 10));
            book.submit(order(2, side, 100, 5));
            check_trades(book.submit(order(3, opposite(side), 100, 4)),
                         {execution(opposite(side), 3, 1, 100, 4)});
            CHECK(book.amend(1, 100, q).empty());
            Snapshot expected;
            auto& levels = side == Side::Buy ? expected.bids : expected.asks;
            if (q == 8)
                levels.push_back({100, 13, {{2, side, 100, 5, 2}, {1, side, 100, 8, 3}}});
            else
                levels.push_back({100, q + 5, {{1, side, 100, q, 1}, {2, side, 100, 5, 2}}});
            check_snapshot(book.snapshot(), expected);
            const auto expectedTrades = q == 8 ? std::vector<Trade>{
                execution(opposite(side), 4, 2, 100, 5), execution(opposite(side), 4, 1, 100, 2)} :
                std::vector<Trade>{execution(opposite(side), 4, 1, 100, q),
                                  execution(opposite(side), 4, 2, 100, 7 - q)};
            check_trades(book.submit(order(4, opposite(side), 100, 7)), expectedTrades);
            CHECK(book.cancel(q == 8 ? 1 : 2));
            check_snapshot(book.snapshot(), Snapshot{});
        }
    }
}

TEST_CASE(amend_increase_at_any_queue_position_gets_new_priority) {
    for (Side side : sides) {
        for (OrderId target : {OrderId{1}, OrderId{2}, OrderId{3}}) {
            CheckedOrderBook book;
            for (OrderId id : {OrderId{1}, OrderId{2}, OrderId{3}})
                book.submit(order(id, side, 100, 2));
            CHECK(book.amend(target, 100, 3).empty());
            Snapshot expected;
            SnapshotLevel level{100, 7, {}};
            std::vector<Trade> trades;
            for (OrderId id : {OrderId{1}, OrderId{2}, OrderId{3}}) {
                if (id == target) continue;
                level.orders.push_back({id, side, 100, 2, id});
                trades.push_back(execution(opposite(side), 4, id, 100, 2));
            }
            level.orders.push_back({target, side, 100, 3, 4});
            (side == Side::Buy ? expected.bids : expected.asks).push_back(level);
            check_snapshot(book.snapshot(), expected);
            check_location(book, target, side, 100, 2);
            trades.push_back(execution(opposite(side), 4, target, 100, 3));
            check_trades(book.submit(order(4, opposite(side), 100, 7)), trades);
            check_snapshot(book.snapshot(), Snapshot{});
        }
    }
}

TEST_CASE(amend_reprice_joins_destination_fifo_and_preserves_source_survivors) {
    for (Side side : sides) {
        for (Price destination : {Price{99}, Price{101}}) {
            for (Quantity q : {Quantity{3}, Quantity{6}, Quantity{8}}) {
                CheckedOrderBook book;
                book.submit(order(1, side, 100, 6));
                book.submit(order(2, side, 100, 5));
                book.submit(order(3, side, destination, 4));
                const auto survivor = OrderBookTestAccess::ids(book).at(2);
                CHECK(book.amend(1, destination, q).empty());
                CHECK(OrderBookTestAccess::ids(book).at(2).order == survivor.order);
                CHECK(survivor.order->priority == 2);
                check_location(book, 2, side, 100);
                check_location(book, 3, side, destination);
                check_location(book, 1, side, destination, 1);
                CHECK(OrderBookTestAccess::ids(book).at(1).order->priority == 4);
                CHECK(book.cancel(2));
                check_trades(book.submit(order(4, opposite(side), destination, 4 + q)), {
                    execution(opposite(side), 4, 3, destination, 4),
                    execution(opposite(side), 4, 1, destination, q)});
                check_snapshot(book.snapshot(), Snapshot{});
            }
        }
    }
...
TEST_CASE(amend_boundary_values_and_full_width_id_recreate_levels) {
    for (Side side : sides) {
        CheckedOrderBook book;
        const OrderId id = std::numeric_limits<OrderId>::max();
        book.submit(order(id, side, 100, 2));
        for (Price p : {Price{1}, max_price}) {
            CHECK(book.amend(id, p, max_quantity).empty());
            Snapshot expected;
            (side == Side::Buy ? expected.bids : expected.asks).push_back(
                {p, max_quantity, {{id, side, p, max_quantity, p == 1 ? Priority{2} : Priority{3}}}});
            check_snapshot(book.snapshot(), expected);
            check_location(book, id, side, p);
        }
        CHECK(book.amend(id, max_price, 1).empty());
        CHECK(OrderBookTestAccess::ids(book).at(id).order->quantity == 1);
        CHECK(OrderBookTestAccess::ids(book).at(id).order->priority == 3);
        CHECK(book.cancel(id));
        check_snapshot(book.snapshot(), Snapshot{});
        book.submit(order(id, opposite(side), 100, 1));
        CHECK(book.cancel(id));
    }
}

TEST_CASE(amend_crossing_sweep_uses_maker_prices_and_cleans_full_fills) {
    for (Side side : sides) {
        for (Quantity q : {Quantity{1}, Quantity{2}, Quantity{3}, Quantity{5}, Quantity{8}, Quantity{9}}) {
            CheckedOrderBook book;
            book.submit(order(1, side, price(side, -2), 6));
            book.submit(order(10, opposite(side), 100, 2));
            book.submit(order(11, opposite(side), 100, 3));
            book.submit(order(12, opposite(side), price(side, 1), 4));
            std::vector<Trade> expectedTrades;
            Quantity left = q;
            Snapshot expected;
            for (const auto& maker : std::array<Order, 3>{{
                     {10, opposite(side), 100, 2, 2}, {11, opposite(side), 100, 3, 3},
                     {12, opposite(side), price(side, 1), 4, 4}}}) {
                const auto filled = std::min(left, maker.quantity);
                if (filled) expectedTrades.push_back(execution(side, 1, maker.id, maker.price, filled));
                left -= filled;
                if (filled == maker.quantity) continue;
                auto& levels = side == Side::Buy ? expected.asks : expected.bids;
                if (levels.empty() || levels.back().price != maker.price)
                    levels.push_back({maker.price, 0, {}});
                auto remainder = maker;
                remainder.quantity -= filled;
                levels.back().quantity += remainder.quantity;
                levels.back().orders.push_back(remainder);
            }
            check_trades(book.amend(1, price(side, 2), q), expectedTrades);
            check_snapshot(book.snapshot(), expected);
            CHECK(!OrderBookTestAccess::ids(book).contains(1));
            CHECK(!book.cancel(1));
            CHECK(OrderBookTestAccess::counter(book) == 4);
            // A fully executed amendment frees its retained ID immediately.
            book.submit(order(1, side, price(side, -2), 1));
            CHECK(book.cancel(1));
            check_snapshot(book.snapshot(), expected);
        }
    }
...
TEST_CASE(amend_sweep_residual_rests_at_new_limit_and_can_be_canceled) {
    for (Side side : sides) {
        CheckedOrderBook book;
        book.submit(order(1, side, price(side, -2), 1));
        book.submit(order(2, opposite(side), 100, 2));
        book.submit(order(3, opposite(side), price(side, 1), 3));
        const Price p = price(side, 2);
        check_trades(book.amend(1, p, 8), {
            execution(side, 1, 2, 100, 2), execution(side, 1, 3, price(side, 1), 3)});
        Snapshot expected;
        (side == Side::Buy ? expected.bids : expected.asks).push_back({p, 3, {{1, side, p, 3, 4}}});
        check_snapshot(book.snapshot(), expected);
        CHECK(OrderBookTestAccess::ids(book).size() == 1);
        check_location(book, 1, side, p);
        CHECK(book.cancel(1));
        check_snapshot(book.snapshot(), Snapshot{});
    }
}

TEST_CASE(amend_sweep_stops_at_limit_and_keeps_worse_maker) {
    for (Side side : sides) {
        CheckedOrderBook book;
        book.submit(order(1, side, price(side, -2), 6));
        book.submit(order(2, opposite(side), 100, 2));
        book.submit(order(3, opposite(side), price(side, 2), 7));
        const auto untouched = OrderBookTestAccess::ids(book).at(3);
        check_trades(book.amend(1, price(side, 1), 5), {execution(side, 1, 2, 100, 2)});
        Snapshot expected;
        (side == Side::Buy ? expected.bids : expected.asks).push_back(
            {price(side, 1), 3, {{1, side, price(side, 1), 3, 4}}});
        (side == Side::Buy ? expected.asks : expected.bids).push_back(
            {price(side, 2), 7, {{3, opposite(side), price(side, 2), 7, 3}}});
        check_snapshot(book.snapshot(), expected);
        CHECK(OrderBookTestAccess::ids(book).at(3).order == untouched.order);
        check_trades(book.amend(1, price(side, 2), 3),
                     {execution(side, 1, 3, price(side, 2), 3)});
        CHECK(!book.cancel(1));
        CHECK(book.cancel(3));
        check_snapshot(book.snapshot(), Snapshot{});
    }
...
TEST_CASE(amend_repeated_changes_account_for_quantity_and_retain_id_ownership) {
    for (Side side : sides) {
        CheckedOrderBook book;
        book.submit(order(1, side, 100, 10));
        book.submit(order(2, side, 100, 5));
        check_trades(book.submit(order(3, opposite(side), 100, 4)),
                     {execution(opposite(side), 3, 1, 100, 4)});
        CHECK(book.amend(1, 100, 8).empty()); // adjustment +2
        CHECK(book.amend(1, 100, 3).empty()); // adjustment -5, stays behind ID 2
        CHECK(book.amend(1, price(side, -1), 4).empty()); // adjustment +1
        CHECK(book.amend(1, 100, 4).empty()); // adjustment 0, returns behind ID 2
        const auto before = book.snapshot();
        expect_rejection([&] { book.submit(order(1, side, 100, 1)); });
        check_snapshot(book.snapshot(), before);
        check_location(book, 1, side, 100, 1);
        CHECK(OrderBookTestAccess::ids(book).at(1).order->priority == 5);
        CHECK(OrderBookTestAccess::ids(book).at(2).order->priority == 2);
...
int main(int argc, char **argv)
{
    ...
    if (argc > 2)
    {
        std::cerr << "Usage: order_book_tests [test_name|--list]\n";
        return 2;
    }
    int passed = 0, failed = 0, skipped = 0;
    for (const auto &item : cases())
    {
        if (argc == 2 && item.name != argv[1])
            continue;
        try
        {
            item.run();
